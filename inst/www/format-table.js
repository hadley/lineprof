...
var cols = [
  ...
  {id: "release", name: "r", field: "release", width: 30, cssClass: "right"},
...
Shiny.addCustomMessageHandler("formatTable",
  function(data) {
    ...
    for (var i = 0; i < data.alloc.length; i++) {
      ...
      d["time"] = percent_bar(data.time[i], max_time, "positive-bar");
      d["release"] = percent_bar(data.release[i], max_release, "negative-bar");
      d["alloc"] = percent_bar(data.alloc[i], max_alloc, "positive-bar");
      d["dups"] = percent_bar(data.dups[i], max_dups, "positive-bar");
    }
